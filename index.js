...
function ortalamaBul(birDizi) {
  ...
  const ortalama = toplam / birDizi.length;
  return ortalama;
}

console.log(ortalamaBul([5, -14, 48, 8]));
...
function ortalamadanBuyukleriBul(birDizi, avgFind) {
  if (birDizi.length === 0) {
    return null
  }
  const yeniDizi = []
  const ortalama = avgFind(birDizi)
  for (let i = 0; i < birDizi.length; i++) {
    if (birDizi[i] >= ortalama) {
...
console.log(ortalamadanBuyukleriBul[5, 18, 20, -34, 7],)
